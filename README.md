...
- Main chat uses a dedicated virtualized waterfall row model with lighter assistant prose, compact user bubbles, distinct tool/file/command audit rows, desktop prompt-floor navigation, a searchable prompt map, data-driven transcript search, bottom-follow behavior, and a Jump to latest control for long transcripts.
